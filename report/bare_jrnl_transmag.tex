...
\begin{figure}[h]
\centering
\includegraphics[width=3.44in]{update.bmp}
% where an .eps filename suffix will be assumed under latex, 
% and a .pdf suffix will be assumed for pdflatex; or what has been declared
% via \DeclareGraphicsExtensions.
...
\multirow{2}{*}{\makecell[c]{SSD}}
 & \makecell[c]{Mask} & \makecell[c]{0.86} & \makecell[c]{0.79} & \makecell[c]{0.21} & \makecell[c]{0.61}\\ \cline{2-6}
 & \makecell[c]{Face} & \makecell[c]{0.81} & \makecell[c]{0.79} & \makecell[c]{0.30} & \makecell[c]{0.64}\\ \hline
...
% and a .pdf suffix will be assumed for pdflatex; or what has been declared
% via \DeclareGraphicsExtensions.
\caption{左侧从上到下分别是口罩目标检测在IoU=0.5、0.7、0.9时的Precision-Recall 曲线；右侧为脸目标检测。}
\end{figure}
\section{实验结果分析}
SSD模型结构：
\begin{figure}[h]
